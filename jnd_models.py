...
def func_JND_with_complexity(img):
    """带模式复杂度的完整JND模型"""
    ...
    print(f"带模式复杂度的JND模型 - MSE = {mse_val:.3f}")

    return img_jnd, jnd_map, jnd_LA, jnd_VM, P_c


def func_JND_without_complexity(img):
    """不带模式复杂度的简化JND模型"""
    if img.dtype != np.float64:
        img = img.astype(np.float64)
    ...
    # 不考虑模式复杂度，直接使用亮度对比度掩蔽作为视觉掩蔽
    # 没有复杂度计算部分，也没有基于复杂度的掩蔽调整
    jnd_VM = jnd_LC  # 关键简化：仅使用亮度对比度掩蔽

    # JND映射（与完整模型相同，但输入的jnd_VM不同）
    jnd_map = jnd_LA + jnd_VM - 0.3 * np.minimum(jnd_LA, jnd_VM)
    ...
    # 计算MSE
    mse_val = np.mean((img_jnd.astype(np.float64) - img) ** 2)
    print(f"不带模式复杂度的JND模型 - MSE = {mse_val:.3f}")

    return img_jnd, jnd_map, jnd_LA, jnd_VM
